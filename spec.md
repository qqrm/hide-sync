...
* Локальное скрытие тредов в интерфейсе сайта.
...
* Работает исключительно на домене **2ch.su** и всех его поддоменах.
  * Использует manifest в формате **DAWAI V3 (Manifest V3 совместимый)**.
* Внешние системы:
...
* Перехватывать действия пользователя "Скрыть тред" и "Показать тред" (в том числе клики по нативным кнопкам сайта **2ch.su**).
  * Каждое действие работает только на уровне треда: отдельные сообщения пользователь может скрывать штатными средствами сайта, но расширение **не** синхронизирует такие изменения.
  * Действия "Скрыть раздел" / "Показать раздел" отсутствуют и не должны реализовываться.
  * Вносить изменения в локальное состояние скрытых элементов.
  * Немедленно отражать изменения в UI (DOM манипуляции).
* Соблюдать ограничение на объем хранилища для **каждого раздела**: задаем фиксированный лимит (например, 150 записей на раздел как верхнюю оценку потребностей пользователя). При попытке добавить новую запись в раздел, где лимит уже достигнут, нужно удалить самые старые записи из **этого же раздела**, чтобы не превышать размер JSON-файла в Gist.
...
* При изменении скрытого состояния:
  ...
  * Через фиксированную задержку ~5 секунд после последнего клика "Скрыть"/"Показать" отправить обновленный JSON в Gist (debounce, чтобы избежать отправки при случайных нажатиях).
...
* Если токен не настроен:
  ...
  * Дополнительные элементы UX (ручная кнопка синхронизации, профили и т.д.) **не требуются**.
...
* Для текущей реализации используется только профиль `default`; поддержка нескольких профилей не требуется.
* `sites` индекс по домену.
* Каждый тред хранится как объект, содержащий:

  * `thread_id` — номер/идентификатор треда, совпадающий с ID на странице 2ch.su.
  * `board_id` — раздел (например, `b`, `pr`), чтобы корректно идентифицировать тред и при необходимости удалять самые старые записи внутри конкретного раздела при достижении его лимита.
  * Идентификаторы тредов определяются логикой маппинга в content скрипте (могут быть получены из DOM, URL, комбинации параметров).
...
* Интеграция с конкретным сайтом:
  ...
  * Нахождение DOM элементов тредов.
  * Присвоение стабильных идентификаторов и определение `board_id` (например, `b`, `pr`) для каждого треда.
  * Добавление UI элемента "Скрыть" / "Показать".
* Применение состояния:

  * Скрывать DOM элементы по спискам `threads` из локального кеша.
* Отправка событий в background:

  * На "Скрыть тред": `HIDE_THREAD { domain, thread_id, board_id }`.
  * На "Показать тред": `UNHIDE_THREAD { domain, thread_id, board_id }`.
...
* Поддержка API для content scripts через `runtime.onMessage`:
  ...
  * `GET_STATE { domain }` → возвращает список `threads` по домену (каждый элемент содержит `thread_id`, `board_id`).
  * `MUTATE_STATE` (hide/unhide) → принимает `domain`, `thread_id`, `board_id`, обновляет локальное состояние и ставит задачу на синхронизацию с Gist.
...
interface SyncStorage {
  ...
  hideThread(domain: DomainId, entry: ThreadEntry): Promise<void>;
  unhideThread(domain: DomainId, entry: ThreadEntry): Promise<void>;
}
```
